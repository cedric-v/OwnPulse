...
The `/prospecting` workspace is designed for a configurable daily outreach goal (10 distinct leads by default). The goal can be changed under **Settings → General → Objectif quotidien de prospection**. It uses the existing contact statuses `Warm`, `Interested`, and `Engaged` for the warm-to-hot KPI card and the "Tièdes à chauds" filter; it does not introduce a duplicate temperature field. `contacts.notes`, `first_contact_date`, `acquisition_channel`, and the existing `tasks` table keep their original meanings.

The daily queue ordering is designed to stay motivating without hammering the same leads: due follow-ups stay at the top, the rest of the queue is reshuffled every day with a date-seeded deterministic shuffle (stable within a given day, different the next morning), and leads contacted within the last 14 days are demoted to the bottom of the list with a "Contacté aujourd'hui" / "Contacté hier" / "Contacté il y a X jours" badge (calendar-day based) and reduced opacity. A dedicated **Contactés récemment** filter shows exactly those recent contacts. Searching still surfaces any lead regardless of these exclusions.

The workflow is explicit: open the available channel, contact the person, then click **J’ai contacté cette personne** and confirm the channel actually used. The log panel includes a **Date du contact** field that defaults to today and can be backdated (not set in the future) for late reporting — e.g. an unlogged call from two days ago — so the contact history, the "Contacté il y a X jours" badges, and the daily goal of the chosen day all reflect the actual contact date. `first_contact_date` likewise uses the chosen date, not the logging date. Available outreach channels are MP LinkedIn, e-mail, WhatsApp, SMS, appel, MP Instagram, Threads, and autre. The quick actions open LinkedIn, e-mail, phone, WhatsApp, or SMS when the corresponding contact data is available.
...
Each contact detail page (`/contacts/<id>`) displays the complete **Prospecting History**: total recorded actions, timestamp, channel, outcome, and note. The **Enregistrer une action** button reuses the prospecting log panel and opens it with the current contact selected, avoiding a second form to maintain. A newly created or imported lead keeps `first_contact_date` empty until a date is explicitly supplied or the first outreach is logged; the interface displays an explicit unknown-date state rather than implying that today is the contact date. Optional follow-up dates likewise remain empty until selected. Search inputs throughout contacts, prospecting, companies, tasks, and duplicate merging are accent- and case-insensitive.
